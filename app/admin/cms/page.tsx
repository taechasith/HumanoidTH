...
export default async function AdminCmsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const localT = {
    en: {
      ...
      deniedDesc: "You must have the ADMIN role to edit CMS content.",
  ...
  const collectionTabs = [
    ...
    { key: "submitted-data" as const, label: localT.submittedData }
  ];
  ...
  return (
    <>
      <div className="topline">
        <div>
          <h1>{t.cmsTitle}</h1>
          <p className="muted">{t.cmsDesc}</p>
        </div>
        <Link className="button primary" href="/admin/submitted-data">
          Review queue
        </Link>
      </div>
      ...
      <div className="toolbar" style={{ marginTop: 0 }}>
        {collectionTabs.map((tab) => (
          <Link
            key={tab.key}
            href={`/admin/cms?collection=${tab.key}${selectedItem ? `&selectedId=${selectedItem.id}` : "&selectedId=new"}`}
            className={`button ${collection === tab.key ? "primary" : ""}`}
          >
            {tab.label}
          </Link>
        ))}
        <Link href={`/admin/cms?collection=${collection}&selectedId=new`} className="button">
          {localT.addNew}
        </Link>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "minmax(0, 1.2fr) minmax(360px, 0.8fr)" }}>
            ...
            <table>
              ...
              <tbody>
                {records.map((record) => (
                  ...
                  >
                    <td>
                      <Link href={`/admin/cms?collection=${collection}&selectedId=${record.id}`} style={{ color: "inherit", display: "block" }}>
                        <strong>
        ...
        <aside className="panel">
          ...
          {collection === "submitted-data" && (
            <form action={upsertSubmittedDataAction} className="form" style={{ marginTop: "12px" }}>
              <input type="hidden" name="id" defaultValue={selectedItem?.id ?? ""} />
              <label>
                Submission Type
